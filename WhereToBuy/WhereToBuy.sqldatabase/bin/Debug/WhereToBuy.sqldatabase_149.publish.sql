...
GO
PRINT N'Creating [dbo].[Step2b1b_Mappings_Status_Map]...';


GO
/*
	This SP, perform Status Mapping of incomplete Quotations.
*/


CREATE PROCEDURE [dbo].[Step2b1b_Mappings_Status_Map]
...
AS
	...
	BEGIN TRY
		...
		*/
		FROM [dbo].[CotacoesIncompletasView] ci
			INNER JOIN [dbo].[EstadosMatching] dm
				ON ci.FornecedorCodigo = dm.FornecedorCodigo
					AND ci._ImpostoCodigo = dm.Codigo
		...
		SET @Info = '[Step2b1b_Mappings_Status_Map-01] Number of mapped quotations: ' + CAST(@@ROWCOUNT as nvarchar);
	...
	BEGIN CATCH
		...
		SET @Info = '[Step2b1b_Mappings_Status_Map-02] ' + CAST(ERROR_NUMBER() as nvarchar) + ' - ' + ERROR_MESSAGE();
