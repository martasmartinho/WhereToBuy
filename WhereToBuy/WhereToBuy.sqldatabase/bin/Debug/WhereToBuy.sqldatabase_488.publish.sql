...
GO
/*
The type for column Notas in table [dbo].[FornecedoresMarcas] is currently  NVARCHAR (260) NULL but is being changed to  NVARCHAR (256) NULL. Data loss could occur.
*/

IF EXISTS (select top 1 1 from [dbo].[FornecedoresMarcas])
    RAISERROR (N'Rows were detected. The schema update is terminating because data loss might occur.', 16, 127) WITH NOWAIT

GO
PRINT N'Altering [dbo].[FornecedoresMarcas]...';


GO
ALTER TABLE [dbo].[FornecedoresMarcas] ALTER COLUMN [Notas] NVARCHAR (256) NULL;
...
AS
	...
	SET @select = 'SELECT Count([Codigo]) FROM [dbo].[FornecedoresMarcas]' 
...
AS
	...
	SET @select = 'SELECT fm.[FornecedorCodigo], fornecedores.[Nome] as FornecedorNome, matching.[Codigo], marcas.[Descricao], fm.[Confianca], 
							fm.[Notas], fm.[Versao]				
						FROM [dbo].[FornecedoresMarcas] fm 
						INNER JOIN [dbo].[Fornecedores] fornecedores
							ON fm.[FornecedorCodigo] = fornecedores.[Codigo]
						INNER JOIN [dbo].[Marcas] marcas
							ON fm.[FornecedorCodigo] = marcas.[Codigo]'
