...
GO
PRINT N'Creating [dbo].[Step3a1_Preparation_ProductMatchingReset]...';
...
CREATE PROCEDURE [dbo].[Step3a1_Preparation_ProductMatchingReset]
...
AS
	...
	BEGIN TRY
		...
		*/
		...
		SET pm.HorasValidadeCotacao = NULL,
			...
			pm.DispensaPrevencaoPrecosDesfazados = CAST('false' as bit),
			...
			pm.Notas =
					CASE WHEN pm.Notas IS NULL THEN '!reset (' + CONVERT(varchar, GETDATE(), 120) + ')'
					     ELSE '!reset (' + CONVERT(varchar, GETDATE(), 120) + ') | ' + pm.Notas
					END,
		...
		*/
		...
		SET @Info = '[Step3a1_Preparation_ProductMatchingReset-01] Number of ProductMatching rules reseted to default: ' + CAST(@@ROWCOUNT as nvarchar);
	...
	BEGIN CATCH
		...
		SET @Info = '[Step3a1_Preparation_ProductMatchingReset-02] ' + CAST(ERROR_NUMBER() as nvarchar) + ' - ' + ERROR_MESSAGE();
