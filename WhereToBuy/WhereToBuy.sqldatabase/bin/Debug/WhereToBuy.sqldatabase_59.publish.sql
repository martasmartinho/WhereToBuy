...
BEGIN
	...
	-- forçar maiusculas
	SET @pn = UPPER(@_Partnumber);

	-- retirar acentuação (neste momento temos a garantia que estão todos em maiusculas)
	SET @pn = REPLACE(@pn, 'À', 'A');
	SET @pn = REPLACE(@pn, 'Á', 'A');
	SET @pn = REPLACE(@pn, 'Ã', 'A');
	SET @pn = REPLACE(@pn, 'Â', 'A');
	SET @pn = REPLACE(@pn, 'Ä', 'A');
	SET @pn = REPLACE(@pn, 'È', 'E');
	SET @pn = REPLACE(@pn, 'É', 'E');
	SET @pn = REPLACE(@pn, 'Ê', 'E');
	SET @pn = REPLACE(@pn, 'Ë', 'E');
	SET @pn = REPLACE(@pn, 'Ì', 'I');
	SET @pn = REPLACE(@pn, 'Í', 'I');
	SET @pn = REPLACE(@pn, 'Î', 'I');
	SET @pn = REPLACE(@pn, 'Ï', 'I');
	SET @pn = REPLACE(@pn, 'Ò', 'O');
	SET @pn = REPLACE(@pn, 'Ó', 'O');
	SET @pn = REPLACE(@pn, 'Õ', 'O');
	SET @pn = REPLACE(@pn, 'Ô', 'O');
	SET @pn = REPLACE(@pn, 'Ö', 'O');
	SET @pn = REPLACE(@pn, 'Ù', 'U');
	SET @pn = REPLACE(@pn, 'Ú', 'U');
	SET @pn = REPLACE(@pn, 'Û', 'U');
	SET @pn = REPLACE(@pn, 'Ü', 'U');
...
AS
	...
	*/
	...
	BEGIN
		SET @_ComplementoDescricao = '(valor não recebido)';
	END
	...
	-- @_StockCodigo									(( CAMPO A PREENCHER AUTOMATICAMENTE CASO NÃO SEJA RECEBIDO ))
	...
	-- @_StockDescricao									(( CAMPO A PREENCHER AUTOMATICAMENTE CASO NÃO SEJA RECEBIDO ))
	IF @_StockDescricao IS NULL
	BEGIN
		SET @_StockDescricao = '(valor não recebido)';
	END
	...
	BEGIN
		SET @_ImpostoDescricao = '(valor não recebido)';
	END
