...
GO
PRINT N'Altering [dbo].[Step2b1z_QuotationsRules_FeedNewRules]...';
...
ALTER PROCEDURE [dbo].[Step2b1z_QuotationsRules_FeedNewRules]
...
AS
	...
	BEGIN TRY
		...
		*/
		...
		INSERT [dbo].[CotacoesRegras] (FornecedorCodigo, MarcaCodigo, CategoriaCodigo, StockCodigo, HorasValidade, StockCodigoSubstituto, DataReset, Notas)
		...
		SET @Info = '[Step2b1z_QuotationsRules_FeedNewRules-01] Number of created rules: ' + CAST(@@ROWCOUNT as nvarchar);
	...
	BEGIN CATCH
		...
		SET @Info = '[Step2b1z_QuotationsRules_FeedNewRules-02] ' + CAST(ERROR_NUMBER() as nvarchar) + ' - ' + ERROR_MESSAGE();
	...
	BEGIN TRY
		...
		*/
		...
		SET ci.Descricao = ci.DescricaoUniformizada,
			...
			-- only update expiration date if the value it isn't yet set.
			-- a supplier can send an expiration date and it will be filled on import task
			ci.ValidadeFormula = 
						CASE WHEN ci.Validade IS NULL THEN ci.FormulaValidadeCalculada 
							 ELSE ci.ValidadeFormula -- keep current value
						END,
			ci.Validade = 
						CASE WHEN ci.Validade IS NULL THEN ci.ValidadeCalculada
							 ELSE ci.Validade -- keep current value
						END,
