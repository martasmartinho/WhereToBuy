...
GO
PRINT N'Altering [dbo].[Categorias]...';


GO
ALTER TABLE [dbo].[Categorias] ALTER COLUMN [PrecoMaximoPermitido] DECIMAL (14, 4) NOT NULL;

ALTER TABLE [dbo].[Categorias] ALTER COLUMN [PrecoMinimoPermitido] DECIMAL (14, 4) NOT NULL;
...
RETURN;
GO
PRINT N'Creating [dbo].[Step4b1_FakeStock_Notifications]...';
...
CREATE PROCEDURE [dbo].[Step4b1_FakeStock_Notifications]
	@Info nvarchar(256) OUTPUT
AS
	...
	BEGIN TRY
		...
		*/
		...
		-- fill output @info with sucess message
		SET @Info = '[Step4b1_FakeStock_Notifications-01] Number of inserted notifications: ' + CAST(@@ROWCOUNT as nvarchar);
	...
	BEGIN CATCH
		-- fill output @info with occurred error message
		SET @Info = '[Step4b1_FakeStock_Notifications-02] ' + CAST(ERROR_NUMBER() as nvarchar) + ' - ' + ERROR_MESSAGE();
...
RETURN;
GO
PRINT N'Creating [dbo].[Step4b2_FakeStock_Execution]...';
...
CREATE PROCEDURE [dbo].[Step4b2_FakeStock_Execution]
	@Info nvarchar(256) OUTPUT
AS
	...
	BEGIN TRY
		...
		*/
		...
		-- fill output @info with sucess message
		SET @Info = '[Step4b2_FakeStock_Execution-01] Number of executed FakeStock preventions: ' + CAST(@@ROWCOUNT as nvarchar);
	...
	BEGIN CATCH
		-- fill output @info with occurred error message
		SET @Info = '[Step4b2_FakeStock_Execution-02] ' + CAST(ERROR_NUMBER() as nvarchar) + ' - ' + ERROR_MESSAGE();
