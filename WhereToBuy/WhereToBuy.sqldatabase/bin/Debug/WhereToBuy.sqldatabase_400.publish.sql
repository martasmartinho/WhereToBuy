...
AS
	...
	BEGIN TRY
		...
		*/
		...
		WHERE p.PrecoCusto <> c.PrecoCusto
			  AND p.PrecoCusto_Data <> CAST(GETDATE() AS DATE)
		...
		WHERE p.StockCodigo <> c.StockCodigoEfetivo
			  AND p.StockCodigo_Data <> CAST(GETDATE() AS DATE)
		...
		WHERE p.Descricao <> c.Descricao
			  OR p.Partnumber <> c.Partnumber
			  OR p.FornecedorCodigo <> c.FornecedorCodigo
		...
		SET @Info = '[Step4c1_Finishing_ProductsUpdate-01] Number of updated products: ' + CAST(@numberOfAffectedRows as nvarchar);
